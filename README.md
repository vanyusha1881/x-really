...
- 🔐 **绑定门禁**：保存配置时强制校验模型是否真正支持图片输入，不支持多模态的模型**不允许绑定**（推文多为图文，文本模型会漏判关键信息）
...
- **绑定门禁**：点击「保存配置」时自动校验模型多模态能力（静态识别 → 已缓存结果 → 自动实测三重判断），不支持图片输入的模型会**直接阻止绑定**并提示原因，从源头保证图文推文可被完整分析。
...
- **配图多模态分析**：Content 脚本抓取推文全部配图 URL（`pbs.twimg.com/media`，最多 4 张）+ 引用推文文本，后台下载转 base64 后以 `image_url` 形式随文本一并发送；若模型报图片不支持错误，自动降级为纯文本重试并在卡片标注
