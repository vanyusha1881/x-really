...
- 🖼 **推文配图多模态分析**：自动抓取推文配图（最多 2 张）转图片输入，AI 核查图文是否相符、是否断章取义 / 伪造 / 移花接木
...
- **配图多模态分析**：Content 脚本抓取推文配图 URL（`pbs.twimg.com/media`，最多 2 张），后台下载转 base64 后以 `image_url` 形式随文本一并发送；若模型报图片不支持错误，自动降级为纯文本重试并在卡片标注
