...
- 🔍 **联网核查**：核查前自动检索 **百度 / Bing / DuckDuckGo**（百度优先，对中文本地新闻覆盖最全），结合 AI 给出权威结论，卡片标注引用来源
...
- **联网核查**：核查前自动从推文提取实体（公司/头衔/人名/事件），派生 2-3 个查询角度（原文 / 去年份前缀 / 实体组合），**百度 → Bing → DDG** 多引擎兜底；搜索结果与核心实体不相关时会明确告知模型"忽略此资料，优先用知识判断"，**避免把搜索失败误判为谣言**
